...
func update_limits(camera_limits: CameraLimits) -> void:
	...
	limit_right = int(camera_limits.position.x) + int(camera_limits.size.x) + FRAME_HORIZONTAL_MARGIN
	limit_top = int(camera_limits.position.y) - FRAME_VERTICAL_MARGIN
	limit_bottom = int(camera_limits.position.y) + int(camera_limits.size.y) + FRAME_VERTICAL_MARGIN