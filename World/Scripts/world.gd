...
func change_levels(entered_door: Door) -> void:
	...
	for door: Door in doors:
		if door.connection != entered_door.connection:
			continue
		var offset = entered_door.get_offset(player)
		player.global_position = door.get_exit_point() - offset
		Events.request_camera_target.emit(player.remote_transform_2d)
